...
@Aspect
@Component
public class RectorSystemLogAspect {
//注入Service用于把日志保存数据库
            ...
            for (Method method : methods) {
                if (method.getName().equals(methodName)) {
                    ...
                    if (clazzs.length == arguments.length) {
                        operationType = method.getAnnotation(RectorSystemLog.class).operationType();
                        operationName = method.getAnnotation(RectorSystemLog.class).operationName();
                        break;
                    }
                }
            }
